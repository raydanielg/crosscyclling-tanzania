...
@section('content')
    <div class="flex flex-col md:flex-row md:items-center justify-between gap-4 mb-8">
        <div>
            <h1 class="text-3xl font-black text-gray-900 tracking-tight">Edit Post</h1>
            <p class="text-gray-500 font-medium">Update existing blog post details.</p>
        </div>
            ...
            <div class="grid grid-cols-1 lg:grid-cols-3 gap-8">
                <div class="lg:col-span-2 space-y-6">
                    <div>
                        ...
                        <input type="text" name="title" value="{{ old('title', $post->title) }}" required class="mt-2 w-full rounded-xl border border-gray-200 px-4 py-2.5 text-sm focus:border-red-600 focus:ring-0" placeholder="Enter title" />
                    ...
                    <div>
                        ...
                        <textarea name="excerpt" rows="3" required class="mt-2 w-full rounded-xl border border-gray-200 px-4 py-2.5 text-sm focus:border-red-600 focus:ring-0" placeholder="Short description...">{{ old('excerpt', $post->excerpt) }}</textarea>
                    ...
                    <div>
                        ...
                        <div class="mt-2">
                            ...
                            <div id="quill-editor" style="height: 400px;" class="rounded-xl border border-gray-200">{!! old('content', $post->content) !!}</div>
                        </div>
                ...
                <div class="space-y-6">
                    <div>
                        ...
                        @if($post->image_path)
                            <div class="mt-2 mb-2">
                                <img src="{{ asset($post->image_path) }}" class="w-full h-32 object-cover rounded-xl border border-gray-200" />
                            </div>
                        @endif
                        <div class="mt-2 p-4 border-2 border-dashed border-gray-200 rounded-2xl text-center">
                            ...
                            <p class="mt-2 text-[10px] text-gray-400 font-bold uppercase">Max 2MB (Leave empty to keep current)</p>
                        </div>
                    ...
                    <div>
                        ...
                        <input type="date" name="published_at" value="{{ old('published_at', $post->published_at ? $post->published_at->format('Y-m-d') : date('Y-m-d')) }}" class="mt-2 w-full rounded-xl border border-gray-200 px-4 py-2.5 text-sm focus:border-red-600 focus:ring-0" />
                    </div>

                    <div class="pt-6">
                        <button type="submit" class="w-full px-6 py-3 bg-red-600 rounded-xl text-sm font-bold text-white shadow-lg shadow-red-600/20 hover:bg-red-700 transition-all">Update Post</button>
                    </div>
                </div>
            </div>
        </form>
